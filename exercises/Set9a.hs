...
countValid = length . filter isValid
  where isValid s = s !! 2 == s !! 4 || s !! 3 == s !! 5
...
sumSuccess xs = if length rs == 0 then Left "no data" else Right $ sum rs
  where rs = rights xs
...
open code (Closed x) | code == x = Open x
open code l = l
...
instance Eq Text where
  (Text x) == (Text y) = clean x == clean y
                        where clean = filter (not . isSpace)
------------------------------------------------------------------------------
